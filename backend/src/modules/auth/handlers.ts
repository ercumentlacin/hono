...
export const authRegisterHandler: AuthHandlerRegister = async (c) => {
    try {
...
export const authLoginHandler: AuthHandlerLogin = async (c) => {
    try {
        const db = await createDbConnection();
        ...
        if (!user) throw new CustomHttpException(
