...
export const checkNewEpisode = async ({
  anime,
  _scrapeLatestEpisodeInfo = scrapeLatestEpisodeInfo,
  _AnimeList = AnimeList,
}: {
  anime: IAnime;
  _scrapeLatestEpisodeInfo?: typeof scrapeLatestEpisodeInfo;
  _AnimeList?: typeof AnimeList;
}) => {
  const latestEpisodeInfo = await _scrapeLatestEpisodeInfo(anime.malId);
  ...
  const animeList = await _AnimeList.findOne({ "animes.malId": anime.malId });
